...
class datasetAereo(torch_data.Dataset):
    ...
    def __getitem__(self, item):
        selected_pc, selected_labels, selected_idx, cloud_ind = self._get_one_sample(item)
         # 1. Normaliza as coordenadas XYZ: centraliza na origem
        selected_pc[:, :3] = selected_pc[:, :3] - np.mean(selected_pc[:, :3], axis=0)
            # 2. Normaliza a Intensidade: garante que está entre 0 e 1
        #    (Esta é uma segurança extra, caso a normalização no pré-processamento tenha falhado)
        #    Pega a quarta coluna (índice 3), encontra o max e min dela, e normaliza.
        intensity_col = selected_pc[:, 3]
        min_i, max_i = np.min(intensity_col), np.max(intensity_col)
        if max_i - min_i > 0:
            selected_pc[:, 3] = (intensity_col - min_i) / (max_i - min_i)
    ...
    def _get_one_sample(self, item):
        ...
        if len(pc) < cfg.num_points:
            ...
            selected_pc = pc[indices]
            selected_labels = labels[indices]
            selected_idx = indices
            selected_pc -= np.mean(selected_pc, axis=0)  # Centraliza a nuvem de pontos
            ...
            selected_pc -= np.mean(selected_pc, axis=0)  # Centraliza a nuvem de pontos
                        ...
                        # --- INÍCIO DO BLOCO DE DATA AUGMENTATION ---
        # Aplica as transformações apenas se estivermos no modo de treinamento
        # 1. PRIMEIRO, recorta a amostra de tamanho fixo a partir dos dados originais
        #    Esta operação usa a 'pc' e 'tree' originais, que são consistentes.
        # 2. DEPOIS, aplica a aumentação de dados APENAS na amostra já recortada
        if self.mode == 'training':
            # Rotação aleatória no eixo Z
            theta = np.random.uniform(0, 2 * np.pi)
            rotation_matrix = np.array([
                [np.cos(theta), -np.sin(theta)],
                [np.sin(theta), np.cos(theta)]
            ])
            selected_pc[:, :2] = selected_pc[:, :2].dot(rotation_matrix)
            
            # Escala aleatória
            scale = np.random.uniform(0.95, 1.05)
            selected_pc = selected_pc * scale
            
            # Jittering (ruído)
            jitter = np.random.normal(0, 0.01, selected_pc.shape)
            selected_pc = selected_pc + jitter
    ...
    def get_data(self, file_path):
        ...
        bin_path = self.dataset_path.parent / 'sequences' / seq_id / 'velodyne' / f'{frame_id}.bin'
        points = np.fromfile(bin_path, dtype=np.float32).reshape(-1, cfg.num_features)
