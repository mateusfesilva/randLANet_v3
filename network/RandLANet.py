...
class Network(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.fc0 = pt_utils.Conv1d(cfg.num_features, 8, kernel_size=1, bn=True)
...
class Building_block(nn.Module):
    ...
    def forward(self, xyz, feature, neigh_idx):  # feature: Batch*channel*npoints*1
        ...
        f_neighbours = Building_block.gather_neighbour(feature.squeeze(-1).permute((0, 2, 1)), neigh_idx)
        ...
        # batch*npoint*nsamples*channel
        f_neighbours = Building_block.gather_neighbour(f_pc_agg.squeeze(-1).permute((0, 2, 1)), neigh_idx)
        f_neighbours = f_neighbours.permute((0, 3, 1, 2))  # batch*channel*npoint*nsamples
        f_concat = torch.cat([f_neighbours, f_xyz], dim=1)
    ...
    def relative_pos_encoding(self, xyz, neigh_idx):
        # xyz aqui pode ter mais de 3 canais (ex: XYZI)
        neighbor_xyz = Building_block.gather_neighbour(xyz, neigh_idx)
        xyz_tile = xyz.unsqueeze(2).repeat(1, 1, neigh_idx.shape[-1], 1)

        # --- CORREÇÃO AQUI ---
        # Garante que todos os cálculos geométricos usem apenas as 3 primeiras colunas (XYZ)
        relative_xyz = xyz_tile[:, :, :, :3] - neighbor_xyz[:, :, :, :3]
        relative_dis = torch.sqrt(torch.sum(torch.pow(relative_xyz, 2), dim=-1, keepdim=True))

        # Concatena as features, usando apenas os XYZ para as coordenadas,
        # garantindo que a saída tenha sempre 10 dimensões (1 + 3 + 3 + 3).
