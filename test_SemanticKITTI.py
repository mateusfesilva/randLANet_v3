...
from dataset_aereo_test import datasetAereoTest, collate_fn_test
from dataset.semkitti_testset import SemanticKITTI
from network.RandLANet import Network
from dataset_aereo_test import datasetAereoTest
import pickle
...
parser.add_argument('--test-path', type=str, required=True, help='caminho para a pasta com os dados de teste')
...
class Tester:
    def __init__(self):
        ...
        test_dataset = datasetAereoTest(test_path=FLAGS.test_path, config=cfg)

        self.test_loader = DataLoader(
            test_dataset,
            batch_size=FLAGS.batch_size, # Usamos o batch_size aqui
            shuffle=False, # Não embaralhar no teste
            num_workers=0,
            collate_fn=collate_fn_test # A collate_fn do seu dataset
        )
        # Network & Optimizer
    ...
    def test(self):
        ...
        save_path = Path(FLAGS.result_dir)
        save_path.mkdir(exist_ok=True)

        # Mapa de cores para as suas 3 classes
        color_map = {
            0: [0.5, 0.5, 0.5], # Classe 0 - Cinza (ex: ground)
            1: [0, 1, 0],       # Classe 1 - Verde (ex: vegetation)
            2: [1, 0, 0],       # Classe 2 - Vermelho (ex: building)
        }

        self.net.eval()
        with torch.no_grad():
            for i, (inputs, file_paths) in enumerate(tqdm(self.test_loader)):
                # 'inputs' é o dicionário para o modelo
                # 'file_paths' é a lista de caminhos que a collate_fn retornou

                # Agora, mova os dados do DICIONÁRIO 'inputs' para a GPU
                for key in inputs:
                ...
                preds = torch.argmax(end_points['logits'], dim=1) # Shape: [B, N]

                # Processar e salvar cada nuvem de pontos do lote
                for j in range(preds.shape[0]):
                    points = inputs['features'][j].cpu().numpy().T # Pega os pontos XYZ
                    pred_labels = preds[j].cpu().numpy()

                    # Cria as cores com base nas previsões
                    colors = np.array([color_map.get(l, [1, 1, 1]) for l in pred_labels])

                    # Cria o objeto PointCloud do Open3D
                    pcd = o3d.geometry.PointCloud()
                    pcd.points = o3d.utility.Vector3dVector(points[:, :3]) # Usa apenas XYZ
                    pcd.colors = o3d.utility.Vector3dVector(colors)

                    # Salva o arquivo .ply
                    file_name = f"pred_{i:03d}_{j:02d}.ply"
                    o3d.io.write_point_cloud(str(save_path / file_name), pcd)

        self.logger.info("Testing finished. Results saved to .ply files.")
