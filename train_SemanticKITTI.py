...
# my module
from dataset_aereo import datasetAereo
...
class Trainer:
    def __init__(self):
        ...
        # Remove handlers existentes para garantir um setup limpo
        if logger.hasHandlers():
            logger.handlers.clear()

        # Adiciona um handler para escrever no arquivo
        file_handler = logging.FileHandler(log_fname)
        file_handler.setFormatter(formatter)
        logger.addHandler(file_handler)

        # (Opcional, mas recomendado) Adiciona um handler para também imprimir no console
        # Assim você vê o log na célula do notebook E no arquivo.
        console_handler = logging.StreamHandler()
        console_handler.setFormatter(formatter)
        logger.addHandler(console_handler)

        # Pega o logger específico para esta classe (boa prática)
        self.logger = logging.getLogger("Trainer")
        self.logger.info("Logger inicializado corretamente.") # Mensagem de teste
        
        # tensorboard writer
        self.tf_writer = SummaryWriter(self.log_dir)
        # get_dataset & dataloader
        train_dataset = datasetAereo('training')
        val_dataset = datasetAereo('validation')
        ...
        self.optimizer = optim.AdamW(self.net.parameters(), lr=cfg.learning_rate, weight_decay=1e-4)
        # self.scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer=self.optimizer, T_max=FLAGS.max_epoch)
        # Scheduler 1: Warm-up linear por uma época (ex: 100 iterações)
        warmup_scheduler = optim.lr_scheduler.LinearLR(self.optimizer, start_factor=0.01, end_factor=1.0, total_iters=len(self.train_loader))

        # Scheduler 2: Cosine Annealing para o resto do treino
        main_scheduler = optim.lr_scheduler.CosineAnnealingLR(self.optimizer, T_max=FLAGS.max_epoch - 1)

        # Combina os dois: o warmup_scheduler roda na primeira época, o main_scheduler depois
        self.scheduler = optim.lr_scheduler.SequentialLR(self.optimizer, schedulers=[warmup_scheduler, main_scheduler], milestones=[1])
        ...
        if CHECKPOINT_PATH is not None and os.path.isfile(CHECKPOINT_PATH):
            if CHECKPOINT_PATH is not None and os.path.isfile(CHECKPOINT_PATH):
                self.logger.info(f"Carregando checkpoint de '{CHECKPOINT_PATH}' para fine-tuning...")
                checkpoint = torch.load(CHECKPOINT_PATH, map_location=torch.device('cpu'))

                pretrained_dict = checkpoint['model_state_dict']
                model_dict = self.net.state_dict()

                # Filtro final e simplificado: remove apenas chaves que começam com 'fc0.'
                pretrained_dict_filtered = {k: v for k, v in pretrained_dict.items() if not k.startswith('fc0.')}

                model_dict.update(pretrained_dict_filtered)

                self.net.load_state_dict(model_dict)

                self.logger.info("Checkpoint carregado com sucesso, exceto a primeira camada (fc0).")
        ...
        self.criterion = nn.CrossEntropyLoss(weight=class_weights, ignore_index=0, reduction='none')
